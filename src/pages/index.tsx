...
export default function Home() {
    ...
    const createRoom = async () => {
        try {
            ...
            peerConnection.current.onicecandidate = event => {
                if (event.candidate) {
                    set(ref(db, `rooms/${roomId}/callerCandidates/${event.candidate.candidate}`), event.candidate.toJSON());
                }
            };
            ...
            onValue(ref(db, `rooms/${roomId}/answer`), async snapshot => {
                ...
                if (answer && peerConnection.current && !peerConnection.current.currentRemoteDescription) {
    ...
    const joinRoom = async () => {
        try {
            ...
            peerConnection.current.onicecandidate = event => {
                if (event.candidate) {
                    set(ref(db, `rooms/${roomId}/calleeCandidates/${event.candidate.candidate}`), event.candidate.toJSON());
                }
            };
    ...
    return (
        <>
            ...
            <Container>
                ...
                {inCall && <StatusMessage type="success">Conectado à sala: {roomId}</StatusMessage>}
