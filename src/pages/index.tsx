...
const LevelMeterFill = styled.div<{ level: number }>`
  ...
  background: ${(props) =>
		props.level > 60 ? "#38a169" : props.level > 30 ? "#dd6b20" : "#c53030"};
...
export default function Home() {
	...
	const setupRemoteAudioMonitoring = useCallback(
		(stream: MediaStream) => {
			...
			const dataArray = new Uint8Array(
				remoteAnalyser.current.frequencyBinCount,
			);
	...
	const registerParticipant = useCallback(async (roomKey: string) => {
		try {
			...
			participantsListenerRef.current = onValue(participantsRef, (snapshot) => {
	...
	}, [checkMicrophoneActivity, cleanupConnection, initializeAudioContext]);
			...
			<Container>
				...
				{activeRoomId && (
					<RoomInfoCard>
						...
						<Badge>
							{isRoomCreator ? "Você é o anfitrião" : "Você é um participante"}
						</Badge>
					</RoomInfoCard>
				)}
				...
				<RemoteAudioContainer>
					...
					<small>
						O áudio continua ativo em segundo plano, mesmo com a tela bloqueada,
						garantindo comunicação constante.
					</small>
				</RemoteAudioContainer>
